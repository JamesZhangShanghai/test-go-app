pipeline {
    ...
    stages {
        stage('UnitTest') {
            steps {
                script {
                    if( sh(script: 'docker run -e "GO111MODULE=off" -e "GOPROXY=https://goproxy.cn" --rm -v $(pwd):/go/src/gowebdemo -w /go/src/gowebdemo golang:1.14.0 /bin/sh -c "/go/src/gowebdemo/rununittest.sh"', returnStatus: true ) != 0 ){
